...
const contact = [
  { icon: cell, data: '714 - 878 - 5117', type: 'phone', url: '714-878-5117' },
  {
    icon: gmail,
...
const experience = [
  ...
  },
  {
    ...
    data: (
      <>
        Built websites for clients using the HubSpot CMS. Created and updated
        custom templates and modules in HubSpot using the Hubl templating
        language and Preact, a performant React alternative. Translated design
        and styling guidelines from the Graphics Design team into attractive and
        responsive web pages using custom Bootstrap themes.
      </>
    ),
  },
  {
    ...
    data: (
      <>
        ...
        invoicing, reporting, and generating work order bill of materials. Built
        simple web applications using JavaScript tools, technologies and
        frameworks such as React, NodeJS, and Express. Helped troubleshoot any
        IT problems for the office and warehouse.
      </>
    ),
        ...
        <div>
          ...
          >
            {contact.map((item, i) => (
              <a
                href={
                  ({ phone: 'tel:', email: 'mailto:' }[item.type] || '') +
                  item.url
                }
                target="_blank"
                rel="noreferrer"
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center' }}>
                  <img
                    src={item.icon}
                    style={{ height: 'auto', width: 16, marginRight: 4 }}
                    alt="contact icon"
                  />
                  <div style={{ position: 'relative', top: 1, fontSize: 14 }}>
                    {item.data}
                  </div>
                </div>
              </a>
    ...
    <div>
      ...
      >
        <h2>
          ...
          >
            <div>
              <div style={{ position: 'relative', zIndex: 2 }}>{company}</div>
              <div
                style={{
